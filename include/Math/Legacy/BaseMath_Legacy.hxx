...
inline double FastDCAHelixVertex(const KFParticle& p, double ref_x, double ref_y, double ref_z, float bz) {

    ROOT::Math::XYZPoint ref{ref_x, ref_y, ref_z};

    double bq{bz * static_cast<double>(p.Q()) * Tree2Secondaries::Const::Kappa};
    ...
    if (std::abs(cbq) < Tree2Secondaries::Const::AbsAlmostZero) return (ref - pca).R();  // protection
...
inline double FastDCAHelixHelix(const KFParticle& p, const KFParticle& q, float bz) {

    double dca_sq{Const::BigNumber};

    // 1 -- find points of closest approach (PCAs) in XY plane //

    double bq1{bz * static_cast<double>(p.Q()) * Tree2Secondaries::Const::Kappa};
    double bq2{bz * static_cast<double>(q.Q()) * Tree2Secondaries::Const::Kappa};
    ...
    for (auto sign : {+1, -1}) {
        ...
        double tmp_sin1{std::sin(tmp_theta1)};
        double tmp_cos1{std::cos(tmp_theta1)};
        ...
        double tmp_sin2{std::sin(tmp_theta2)};
        double tmp_cos2{std::cos(tmp_theta2)};
    ...
    if (std::abs(detp) < Tree2Secondaries::Const::AbsAlmostZero || detp * detp < Tree2Secondaries::Const::AbsAlmostZero) {
