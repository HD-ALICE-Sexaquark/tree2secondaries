...
inline double CosinePointingAngle(const ROOT::Math::XYZVector& mom_v0, const ROOT::Math::XYZPoint& xyz_v0, const ROOT::Math::XYZPoint& xyz_ref) {
    auto dir = xyz_v0 - xyz_ref;
    return mom_v0.Dot(dir) / (mom_v0.R() * dir.R());
}
...
inline double ArmenterosAlpha(const ROOT::Math::XYZVector& mom_v0, const ROOT::Math::XYZVector& mom_neg, const ROOT::Math::XYZVector& mom_pos) {
    ...
    if (p_v0 < Const::AbsAlmostZero) [[unlikely]] {
        return Const::BigNumber;  // protection
    }
    double lQlNeg = mom_neg.Dot(mom_v0) / p_v0;
    double lQlPos = mom_pos.Dot(mom_v0) / p_v0;
    if (std::abs(lQlPos + lQlNeg) < Const::AbsAlmostZero) return Const::BigNumber;  // protection
...
inline double FastDCALineVertex(const ROOT::Math::XYZVector& mom_v0, const ROOT::Math::XYZPoint& xyz_v0, const ROOT::Math::XYZPoint& xyz_ref) {
    double p_v0 = mom_v0.R();
    if (p_v0 < Const::AbsAlmostZero) [[unlikely]] {
        return Const::BigNumber;  // protection
    }
    return mom_v0.Cross(xyz_v0 - xyz_ref).R() / p_v0;
}
