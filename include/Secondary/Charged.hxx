...
class Charged {
   public:
    ...
    [[nodiscard]] ROOT::Math::XYZPoint FastXYZ(double theta, double sin_theta, double cos_theta, double omega, double one_over_omega,
    ...
    [[nodiscard]] double Px(double s, const Helper::Propagator &prop) const { return prop.ChargedPx(s, Charge(), Px0(), Py0()); }
    [[nodiscard]] double Py(double s, const Helper::Propagator &prop) const { return prop.ChargedPy(s, Charge(), Px0(), Py0()); }
    [[nodiscard]] double FastPx(double sin_theta, double cos_theta, const Helper::Propagator &prop) const {
        return prop.FastChargedPx(sin_theta, cos_theta, Px0(), Py0());
    }
    [[nodiscard]] double FastPy(double sin_theta, double cos_theta, const Helper::Propagator &prop) const {
